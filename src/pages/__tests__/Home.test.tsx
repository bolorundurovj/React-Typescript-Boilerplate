...
describe('Home', () => {
  it('renders hero section with title and description', () => {
    ...
    expect(
      screen.getByText('React TypeScript Boilerplate')
    ).toBeInTheDocument();
    expect(
      screen.getByText(/A minimal and flexible boilerplate/)
    ).toBeInTheDocument();
  });
  ...
  it('renders features section with title', () => {
    ...
    expect(
      screen.getByText('Everything you need to build modern web apps')
    ).toBeInTheDocument();
  });
  ...
  it('renders feature descriptions', () => {
    ...
    expect(
      screen.getByText(/Built with Vite for lightning-fast development/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Full TypeScript support with strict type checking/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /React 18, TypeScript, Tailwind CSS, and shadcn\/ui components/
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Pre-built components with dark mode support/)
    ).toBeInTheDocument();
  });
  ...
  it('renders CTA section', () => {
    ...
    expect(
      screen.getByText(/Clone this repository and start building/)
    ).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: /clone repository/i })
    ).toBeInTheDocument();
  });
  ...
  it('renders feature icons', () => {
    ...
    const featureCards = screen.getAllByText(
      /Fast Development|Type Safe|Modern Stack|Beautiful UI/
    );
